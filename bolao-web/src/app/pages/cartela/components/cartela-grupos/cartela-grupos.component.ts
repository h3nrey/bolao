...
import { CartelaTableComponent } from '../cartela-table/cartela-table.component';
...
@Component({
  selector: 'app-cartela-grupos',
  standalone: true,
  imports: [CommonModule, FormsModule, CartelaTableComponent, CartelaMatchComponent, RoundSelectorComponent],
  templateUrl: './cartela-grupos.component.html',
})
export class CartelaGruposComponent implements OnInit {
  ...
  // Active Group tab signal
  protected readonly activeGroup = signal<string>('Grupo A');
  ...
  // Filter sections to only show the active group
  protected readonly filteredGroupSections = computed(() => {
    return this.groupSections().filter(g => g.name === this.activeGroup());
  });

  // List of all group names
  protected readonly groupNames = computed(() => {
    return this.groupSections().map(g => g.name);
  });
  ...
  protected fetchData(): void {
    ...
    forkJoin({
      ...
      next: ({ matchesList, myPredictions }) => {
        ...
        // Auto-select the first available group name if activeGroup is not in list
        const sections = this.groupSections();
        if (sections.length > 0 && !sections.some(s => s.name === this.activeGroup())) {
          this.activeGroup.set(sections[0].name);
        }
        
        this.loading.set(false);
      },
      error: (err) => {
        console.error('Falha ao carregar dados da cartela de grupos', err);
        this.loading.set(false);
      }
    });
  }
